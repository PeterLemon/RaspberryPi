; Raspberry Pi 'Bare Metal' V3D VG Triangle Control List Demo by krom (Peter Lemon):
...
mov r1,r0 ; Test To Manually Fill Screen/Tile Data With Red Pixels (If The Screen Is RED The GFX Pipeline Has Crashed)
mov r2,$400000 / 4
imm32 r3,$FF0000FF
LoopRed:
  str r3,[r1],4
...
CONTROL_LIST_RENDER_STRUCT: ; Control List Of Concatenated Control Records & Data Structures (Rendering Mode Thread 1)
  Clear_Colors $FF00FF00FF00FF00, 0, 0, 0 ; Clear Colors (R) (Clear Color (Green/Green), Clear ZS (Green), Clear VGMask, Clear Stencil)
  ...
  Configuration_Bits Enable_Forward_Facing_Primitive + Enable_Reverse_Facing_Primitive + Clockwise_Primitives, 0 ; Configuration Bits (Data8, Data16)

  VG_Shader_State VG_SHADER_STATE_RECORD ; VG Shader State (Base Address)
  VG_Inline_Primitives Primitives_Type_Triangles ; VG Inline Primitives (Only For Use In VG Shader Mode) (Data)
    dh	  0,   0 ; 1st Vertex: Xs =   0, Ys =   0 (Xs In Bits[15:0], Ys In Bits[31:16])
    dh	639,   0 ; 2nd Vertex: Xs = 639, Ys =   0 (Xs In Bits[15:0], Ys In Bits[31:16])
    dh	639, 479 ; 3rd Vertex: Xs = 639, Ys = 479 (Xs In Bits[15:0], Ys In Bits[31:16])
    dw $BFFF0000 ; 3rd Vertex: Special Termination Code Value (Triangles are Terminated By The Code In The 3rd Vertex)

  ; Tile Row 0
  Tile_Coordinates 0, 0 ; Tile Coordinates (R) (Tile Column, Tile Row) ; This tile is always Garbage
...
align 16
VG_SHADER_STATE_RECORD: ; VG Shader State Record
  db 0 ; Flag Bits: 0 = Fragment Shader Is Single Threaded
  db 0 ; Not Used
  db 2 ; Fragment Shader Number Of Uniforms (Not Used Currently)
...
FRAGMENT_SHADER_CODE:
  dd $4000000000000000 ; Wait For Scoreboard (Stall Until This QPU Can Safely Access Tile Buffer)
  dd $E0000000FFFFFFFF ; Load Immediate 32 = $FFFFFFFF (Color White)
  dd $3000000000000000 ; Program End (Thread End)

align 16 ; 128-Bit Align
FRAGMENT_SHADER_UNIFORMS:
  dw $FFFFFFFF ; Uniform 0 Color1 White
  dw $FFFFFFFF ; Uniform 1 Color2 White