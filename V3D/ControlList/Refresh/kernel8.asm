; Raspberry Pi 3 'Bare Metal' V3D Refresh Multi Sample Vertex Color NV Vertex Array Triangle Control List Demo by krom (Peter Lemon):
...
Refresh:
  ...
  mov w1,1
  str w1,[x0,V3D_BFC] ; Reset Flush Count
  str w1,[x0,V3D_RFC] ; Reset Frame Count
  ...
  WaitRenderControlList: ; Wait For Control List To Execute
    ldr w1,[x0,V3D_RFC] ; Load Frame Count
    tst w1,1 ; Test IF Last Tile Store Operation Of The Frame Is Complete
    b.eq WaitRenderControlList
  ...
  cmp w1,160 * 16
  b.ne XINC
  mov w2,1 ; Translation X Direction Increment
  XINC:
  add w1,w1,w2
  strh w1,[x0],6 * 4
  ...
  cmp w1,w3
  b.ne YINC
  mov w2,0 ; Translation X Direction Decrement
  mvn w2,w2
  YINC:
  add w1,w1,w2
  strh w1,[x0]
